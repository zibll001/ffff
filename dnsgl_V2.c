...
// 保存过滤结果到文件 - 高性能版本：使用系统命令高效去重和排序，只输出纯IP
void save_filtered_results(const char *filename, AmplificationResult *results, int count, 
                          double min_amplification, int min_response_size, 
                          int use_amp_filter, int use_size_filter) {
    printf("正在保存过滤结果...\n");
    ...
    // 使用系统命令进行高效去重和排序，只提取IP
    printf("正在使用系统命令去重、排序并提取纯IP...\n");
    ...
    snprintf(command, sizeof(command),
        "sort -t'|' -k1,1nr %s | awk -F'|' '!seen[$3]++ {print $3}' > %s",
        temp_file1, filename);
    ...
    if (ret != 0) {
        ...
        if (ret == 0) {
            snprintf(command, sizeof(command),
                "awk -F'|' '!seen[$3]++ {print $3}' %s > %s", temp_file2, filename);
            ret = system(command);
        }
        ...
        if (ret != 0) {
            ...
            // 最后备用：简单去重
            snprintf(command, sizeof(command),
                "awk -F'|' '!seen[$3]++ {print $3}' %s > %s", temp_file1, filename);
            system(command);
        }
    }
    ...
    printf("高效处理完成，保存了 %d 个唯一IP\n", unique_count);
    
    // 显示前10个最佳结果（从原始数据中计算，保持详细格式）
    printf("\n前10个最佳结果预览:\n");
    printf("%-15s %-8s %-8s %-8s %-20s %-8s\n", 
           "DNS服务器", "倍数", "响应", "请求", "域名", "时间(ms)");
    printf("------------------------------------------------------------------------\n");
    
    // 创建临时文件用于排序和显示前10个详细结果
    char detail_temp[256];
    snprintf(detail_temp, sizeof(detail_temp), "%s.detail", filename);
    
    FILE *detail_file = fopen(detail_temp, "w");
    if (detail_file) {
        for (int i = 0; i < count; i++) {
            if (results[i].success) {
                int meets_amp_criteria = !use_amp_filter || results[i].amplification >= min_amplification;
                ...
                if (meets_amp_criteria && meets_size_criteria) {
                    fprintf(detail_file, "%.2f|%d|%s|%s|%d|%.2f\n", 
                            results[i].amplification,
                            results[i].response_size,
                            results[i].dns_server,
                            results[i].domain,
        ...
        snprintf(sort_cmd, sizeof(sort_cmd),
            "sort -t'|' -k1,1nr %s | head -10", detail_temp);
        
        FILE *sort_pipe = popen(sort_cmd, "r");
        if (sort_pipe) {
            char line[256];
            int preview_count = 0;
            while (fgets(line, sizeof(line), sort_pipe) && preview_count < 10) {
